...
- **FR-016**: Migration dry-run MUST produce a machine-readable report containing, for every row, the legacy and candidate identifiers, classification, decision reason, legacy resolution path, provenance summary, dependent reference counts, and the planned action.
...
- **AC-006**: Every in-scope legacy row is classified in dry-run as `migratable`, `consolidate`, or `unmigratable`, and cutover proceeds only when `unmigratable = 0` and `conflict_groups = 0`.
